// Emits L0178's public static assets into dist/static/. As a child of L0000, L0178 merges
// inherited content from its parent:
//   - lexicon.json: the merged lexicon (base + L0178) — already merged in src/lexicon.ts.
//     (the legacy lexicon.js request path is aliased to it by the API server.)
//   - instructions.md: parent (L0000) instructions concatenated with L0178's.
// The rest (spec.html, language-info.json, scope.json, schema.json, template.gc,
// usage-guide.md) are L0178's own.
...
// 1. lexicon — merged (base + L0178) as plain JSON in lexicon.json. No lexicon.js is written:
...
// 3. instructions.md — parent (L0000) instructions + L0178's. Resolve the parent's
