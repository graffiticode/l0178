...
docker build -t gcr.io/YOUR_PROJECT_ID/l0178:latest .

# Push to Container Registry
docker push gcr.io/YOUR_PROJECT_ID/l0178:latest

# Deploy to Cloud Run
gcloud run deploy l0178 \
  --image gcr.io/YOUR_PROJECT_ID/l0178:latest \
  --platform managed \
  --region us-central1 \
  --port 50178 \
...
- `PORT`: Automatically set by Cloud Run (the app defaults to 50178)
...
```bash
gcloud run services update l0178 \
  --region us-central1 \
...
gcloud run services update-traffic l0178 \
  --region us-central1 \
  --to-latest
...
1. **Port Configuration**: Ensure the app uses `process.env.PORT` and falls back to 50178
2. **Memory Limits**: Cloud Run defaults to 256MB. Increase if needed:
   ```bash
   gcloud run services update l0178 --memory 512Mi --region us-central1
   ```
3. **Cold Starts**: Cloud Run may have cold starts. Consider minimum instances:
   ```bash
   gcloud run services update l0178 --min-instances 1 --region us-central1
   ```
...
```bash
# List revisions
gcloud run revisions list --service l0178 --region us-central1
